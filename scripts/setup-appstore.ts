...
// Apple API credentials (same as ChatFaster)
...
async function getJWT() {
  if (!fs.existsSync(keyPath)) {
    ...
    console.error('');
    console.error('Please copy the key file:');
    console.error('  mkdir -p .keys');
    console.error('  cp /Users/ash/Develop/sass/chatfaster/frontend/.keys/AuthKey_C2W4CXZYKM.p8 .keys/');
    process.exit(1);
  }
...
async function fetchAPI(endpoint: string, jwt: string, options: RequestInit = {}): Promise<{ response: Response, data: any }> {
...
async function getBundleId(jwt: string): Promise<string | null> {
...
async function getCertificateId(jwt: string): Promise<string | null> {
...
async function getExistingProfile(jwt: string): Promise<any | null> {
...
async function createProfile(jwt: string, bundleIdResourceId: string, certificateId: string) {
...
async function downloadProfile(profile: any) {
